...
import reverb
import my_reverb_checkpointer as mrc
...
def make_distributed_experiment(
    ...
    *,
    ...
    reverb_ckpt_path: Optional[str] = None,
    reverb_fallback_path: Optional[str] = None,
    stabi=False
) -> lp.Program:
  ...
  Args:
    ...
    program: a program where agent nodes are added to. If None, a new program is
      created.
    reverb_ckpt_path: path from which to load (unless DNE) and to which to save
    reverb_fallback_path: fallback path from which to load reverb; not to save
  ...
  def build_replay():
    ...
    spec = (
        ...
        specs.make_environment_spec(experiment.environment_factory(1)(dummy_seed)))
    ...
    policy = config.make_policy(
        ...
        evaluation=False)
    tables = experiment.builder.make_replay_tables(spec, policy)
    tables,_ = _disable_insert_blocking(tables)
    return tables

  def build_model_saver(variable_source: core.VariableSource):
    assert experiment.checkpointing
    environment = experiment.environment_factory(1)(utils.sample_uint32(0))
  ...
  def build_inference_server(
    ...
    spec = (
        experiment.environment_spec or
        specs.make_environment_spec(experiment.environment_factory(1)(utils.sample_uint32(dummy_seed))))
    networks = experiment.network_factory(spec)
    policy = config.make_policy(
        experiment=experiment,
        networks=networks,
  ...
  def build_actor(
    ...
    environment = experiment.environment_factory(actor_id)(utils.sample_uint32(environment_key))
  ...
  if reverb_ckpt_path is not None or reverb_fallback_path is not None:
    checkpoint_ctor = mrc.make_reverb_checkpointer(reverb_ckpt_path, reverb_fallback_path)
    replay_node = lp.ReverbNode(
      build_replay,
      checkpoint_time_delta_minutes=checkpoint_time_delta_minutes,
      checkpoint_ctor=checkpoint_ctor)
  else:
    replay_node = lp.ReverbNode(
      build_replay,
      checkpoint_time_delta_minutes=checkpoint_time_delta_minutes)
  ...
  if experiment.max_num_actor_steps is not None:
    program.add_node(
        lp.CourierNode(lp_utils.StepsLimiter, counter,
                       experiment.max_num_actor_steps, 'evaluator_steps'),
        label='counter')
  ...
  with program.group('actor'):
    ...
    *actor_keys, key = jax.random.split(key, num_actors + 1)
    ...
    for aid, akey in enumerate(actor_keys):
      actor_nodes.append(
          lp.CourierNode(
              experiment.get_evaluator_factories(aid+offset)[0],
              akey,
              # replay,
...
def _disable_insert_blocking(tables):
  import sys
